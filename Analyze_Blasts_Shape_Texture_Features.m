...
clear all
clc
addpath('./dependent functions')
masks = dir(['./AML/AMLcGAN_Output/SwissSamplesTiles_result/masks_alltogether/Cleaned/*.tif']);
imgDir = './Original_Tiles';


filename = 'featurs_texture_AMLcGAN.xlsx'; % output file

mask = imresize(imread([masks(1).folder filesep masks(1).name]),[2048 2048]);
img = imread([imgDir filesep masks(1).name(1:end-length('_mask.tif')) '.tif']);
...
for b = 2:length(masks)
    mask = imresize(imread([masks(b).folder filesep masks(b).name]),[2048 2048]);
    img = imread([imgDir filesep masks(b).name(1:end-length('_mask.tif')) '.tif']);
...
filename = 'validation_blast_stat.xlsx'; % output file
